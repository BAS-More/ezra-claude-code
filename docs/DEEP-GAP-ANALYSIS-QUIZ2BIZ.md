...
1. **GAP-004** — Update CLAUDE.md to list all 22 commands
2. **GAP-016** — Fix env var name in INTEGRATION-PLAN-v3
3. **GAP-008** — Archive Railway docs or create Azure deployment docs
4. **GAP-017** — Add Node.js version context note to GETTING_STARTED.md

### Medium-term (next sprint)

1. **GAP-012** — Implement or defer git hooks to v5.1.0
2. **GAP-013** — Implement or defer integration health pillar
3. **GAP-005** — Update dash hook documentation
4. **GAP-006** — Demonstrate all 12 step types or clarify
5. **GAP-010** — Document YAML parser limitations
6. **GAP-011** — Label webhook spec as SPECIFICATION ONLY

### Low priority

1. **GAP-003** — Sync agent source files with installed versions
2. **GAP-007** — Update test expected command count
3. **GAP-009** — Clean up or document ~/.claude/agents/ target
